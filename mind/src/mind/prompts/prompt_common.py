...
def create_prompt_template(
    system_prompt: str | list[str], user_prompt: str | list[str]
) -> ChatPromptTemplate:
...
class TagPattern:
    ...
    def _parse_contents(self, value: str) -> str | T:
        ...
        """
        if self.parser:
            return self.parser(value)  # type: ignore[return-value]
...
class Prompt:
    ...
    def __init__(
        ...
        if any(pattern.name == self.RESPONSE_KEY for pattern in self.output_tag_patterns):
            raise ValueError(
                f"Output tag patterns cannot have the reserved name '{self.RESPONSE_KEY}'."
            )
