...
from mcp.server import Server as CoreMCPServer
...
class MCPServer:
    ...
    def _register_tools_and_resources(self):
        ...
        async def decide_action(
            ...
            """Decide on an action based on observation and available actions
        ...
        # Register resources
        @self.mcp.resource("agent://{agent_id}/mental_state")
        async def get_agent_mental_state(agent_id: str) -> str:
            """Get the agent's complete mental state including memory and personality"""
            try:
                agent = self.agent_manager.get_agent(agent_id)
                
                # Get recent long-term memories
                recent_memories = []
                if hasattr(agent.long_term_memory, 'memories'):
                    recent_memories = agent.long_term_memory.memories[-10:]  # Last 10 memories
                ...
                mental_state = {
                    ...
                    "memory": {
                        ...
                        "long_term_recent": recent_memories,
                        "long_term_count": len(agent.long_term_memory.memories) if hasattr(agent.long_term_memory, 'memories') else 0
                    },
                    "cognitive_state": {
                        "last_observation": agent.current_observation,
                        "last_action_set": agent.current_actions
                    }
                }
                
                return json.dumps(mental_state)
            except Exception as e:
                return json.dumps({
                    "error": str(e)
                })
